...
def main() -> int:
    ...
    for path in sorted(POOL.glob("*/DESIGN.md")):
        ...
        fixture = {"brand": slug, "source": UPSTREAM.format(slug=name),
                   ...
                   "upstream_sha256": hashlib.sha256(up.encode("utf-8")).hexdigest() if up is not None else None,
    ...
    anchors = json.loads(RECEIPT.read_text(encoding="utf-8")).get("hand_anchors", []) if RECEIPT.exists() else []
